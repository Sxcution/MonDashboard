...
function appendPlainTextWithBreaks(target: HTMLElement, text: string): void {
...
function appendRainbowTextWithBreaks(target: HTMLElement, text: string): void {
...
function textLayerIsGradientFill(fill: string): boolean {
    return /^linear-gradient\(/i.test((fill || '').trim());
}

function getTextLayerFill(layer: ImageTextLayer): string {
    return String(layer.fill || layer.color || '#ffffff');
}

function textLayerFirstColor(fill: string, fallback = '#ffffff'): string {
    return (fill || '').match(/#[0-9a-fA-F]{3,8}|rgba?\([^)]+\)/)?.[0] || fill || fallback;
}

function applyTextLayerFillStyle(target: HTMLElement, layer: ImageTextLayer): void {
...
function setupSubmenuHovers(menuX, menuY) {
    ...
    colorMenuItem.onmouseenter = () => {
        ...
        // Set current values
        const colorInput = document.getElementById('colorCodeInput') as HTMLInputElement | null;
        const rainbowInput = document.getElementById('rainbowCheckbox') as HTMLInputElement | null;
        if (colorInput) colorInput.value = currentEditingLayer.color;
        if (rainbowInput) rainbowInput.checked = Boolean(currentEditingLayer.rainbow);
    };
...
function applyTextFillFromCollagePanel(fill: string): boolean {
