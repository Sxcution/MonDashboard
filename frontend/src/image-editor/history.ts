...
function initImageFolderPanel(): void {
    ...
    const imageFolderPathInput = document.getElementById('imageFolderPathInput') as HTMLInputElement | null;
    ...
    if (btnOpenFolderModal) {
        btnOpenFolderModal.addEventListener('click', () => {
            ...
            if (modalEl && typeof bootstrap !== 'undefined') {
                ...
                if (imageFolderPathInput) imageFolderPathInput.value = savedPath;
    ...
    document.addEventListener('click', (e) => {
        ...
        const folderItem = target.closest<HTMLElement>('.image-folder-item');
        if (folderItem) {
        ...
        if (folderItem) {
            ...
            if (itemType === 'folder') {
                ...
                if (filePath) {
                    ...
                    showFileContextMenu(e, filePath, itemType as 'image' | 'video');
                }
            }
        }
    });
...
function showFolderContextMenu(event: MouseEvent, folderPath: string): void {
    const existing = document.querySelector('.context-menu');
    if (existing) existing.remove();
...
function getDisplayName(name: string, type: 'folder' | 'image' | 'video'): string {
    if (type === 'image') {
...
async function loadFileToCanvas(filePath: string, type: 'image' | 'video'): Promise<void> {
    try {
        const serveUrl = `/image/api/serve-file?path=${encodeURIComponent(filePath)}`;
        
        // Deactivate active tools & clean state
        deactivateAllTools();
        if (typeof (window as any).resetObjectRemoveState === 'function') {
            (window as any).resetObjectRemoveState();
        }
        textLayers = [];
        ...
        if (type === 'image') {
            ...
            img.onload = () => {
                ...
                if (collageCanvasContainer) collageCanvasContainer.style.display = 'none';
                ...
                (window as any).currentOpenedFilePath = filePath;
                saveImageToCache([serveUrl], 'collage');
                showSingleImageViewer(img);
            };
            img.onerror = () => {
                (window as any).currentOpenedFilePath = null;
            ...
            video.onloadeddata = () => {
                ...
                (window as any).currentOpenedFilePath = filePath;
                saveImageToCache([serveUrl], 'collage');
                showSingleImageViewer(video as any);
            };
            video.onerror = () => {
                (window as any).currentOpenedFilePath = null;
...
async function fetchAndRenderSubfolders(path: string): Promise<void> {
    try {
        ...
        const previouslySelectedPaths = new Set<string>();
        container.querySelectorAll('.selected-item').forEach(item => {
            const path = (item as HTMLElement).dataset.filePath || (item as HTMLElement).dataset.folderPath;
            if (path) {
        ...
        (window as any).fetchAndRenderSubfolders = fetchAndRenderSubfolders;
    } catch (e) {
        showToast('Lỗi kết nối máy chủ', 'danger');
    }
}

function showFileContextMenu(event: MouseEvent, filePath: string, itemType: 'image' | 'video'): void {
    const existing = document.querySelector('.context-menu');
    if (existing) existing.remove();
    ...
    if (btnDelete) {
        btnDelete.addEventListener('click', async () => {
            ...
            const isConfirmed = await (window as any).showConfirm(confirmMsg, 'Xác nhận xoá file');
