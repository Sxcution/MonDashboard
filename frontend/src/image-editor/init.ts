...
function drawTextLayerAt(ctx: CanvasRenderingContext2D, layer: ImageTextLayer, x: number, y: number, fontSize: number): void {
...
function drawCanvasTextLayer(ctx: CanvasRenderingContext2D, layer: ImageTextLayer, canvasWidth: number, canvasHeight = canvasWidth): void {
...
function drawSingleImageTextLayer(ctx: CanvasRenderingContext2D, layer: ImageTextLayer): void {
...
function bindImageTemplateActions() {
    ...
    document.addEventListener('click', (event) => {
        ...
        const upscaleModelEl = target.closest<HTMLElement>('[data-upscale-model]');
        if (upscaleModelEl) {
