...
function bindLockedDrag(img: HTMLImageElement, imageIndex?: number): void {
    if (!img) return;
    const dragSurface = (img.parentElement || img) as HTMLElement;
    ...
    dragSurface.addEventListener('pointermove', (e) => {
        if (!panDragging && !swapDragging) return;
    ...
    const end = (e?: PointerEvent) => {
        ...
        if (shouldSwap && e) {
            const dropTarget = document
                ...
                ?.closest<HTMLElement>('.tile[data-collage-tile-index]');
...
function swapCollageImageOrder(fromIndex: number, toIndex: number): void {
    if (
        fromIndex < 0 ||
        toIndex < 0 ||
...
let collageColorTarget: 'background' | 'border' | 'text' = 'background';
...
function collageInput(id: string): HTMLInputElement | HTMLSelectElement | null {
    return document.getElementById(id) as HTMLInputElement | HTMLSelectElement | null;
}

function collageIsGradientFill(fill: string): boolean {
    return /^linear-gradient\(/i.test((fill || '').trim());
}

function collageExtractFillColors(fill: string): string[] {
    return (fill || '').match(/#[0-9a-fA-F]{3,8}|rgba?\([^)]+\)/g) || [];
}

function collageCreateCanvasFill(ctx: CanvasRenderingContext2D, fill: string, x: number, y: number, w: number, h: number): string | CanvasGradient {
    const value = (fill || '').trim();
    if (!collageIsGradientFill(value)) return value || '#111111';

    const colors = collageExtractFillColors(value);
    if (colors.length < 2) return colors[0] || '#111111';
...
function collageGetFill(target: 'background' | 'border'): string {
...
function collageSetFill(target: 'background' | 'border', fill: string): void {
    const fillInput = collageInput(target === 'background' ? 'collageBackgroundFill' : 'collageBorderFill');
    const colorInput = collageInput(target === 'background' ? 'collageBackground' : 'collageBorderColor') as HTMLInputElement | null;

    if (fillInput) fillInput.value = fill;
    if (colorInput && !collageIsGradientFill(fill)) colorInput.value = fill;
...
function collageNormalizeHexColor(value: string): string | null {
...
function collageApplyPanelFill(fill: string): void {
...
function collageUpdateGradientPreview(): string {
    ...
    const preview = document.getElementById('collageGradientPreview') as HTMLElement | null;
    if (preview) preview.style.background = gradient;
    return gradient;
}

function collageSyncGradientStops(fill: string): void {
...
function setCollageQuickPanelVisible(visible: boolean): void {
    const panel = document.getElementById('collageQuickPanel') as HTMLElement | null;
    if (panel) panel.style.display = visible ? 'block' : 'none';
    if (!visible) closeCollagePopovers();
}
...
function updateCollageQuickPanelValues(): void {
    ...
    if (aspectValue) aspectValue.textContent = aspect;

    const spaceValue = document.getElementById('collageSpaceQuickValue');
    if (spaceValue) spaceValue.textContent = gutter;

    const swatch = document.getElementById('collageColorQuickSwatch') as HTMLElement | null;
    if (swatch) swatch.style.background = currentFill;

    const cornerIcon = document.getElementById('collageCornerQuickIcon') as HTMLElement | null;
    if (cornerIcon) cornerIcon.style.setProperty('--corner-preview-radius', `${Math.min(parseInt(radius, 10) || 0, 18)}px`);
    ...
    if (spaceMirror) spaceMirror.value = gutter;
    if (borderMirror) borderMirror.value = border;
    if (cornerMirror) cornerMirror.value = radius;
    ...
    if (spaceMirrorValue) spaceMirrorValue.textContent = gutter;
    if (borderMirrorValue) borderMirrorValue.textContent = border;
    if (cornerMirrorValue) cornerMirrorValue.textContent = radius;

    document.querySelectorAll<HTMLElement>('[data-collage-aspect-option]').forEach(button => {
        button.classList.toggle('is-active', button.dataset.collageAspectOption === aspect);
    });
    document.querySelectorAll<HTMLElement>('[data-collage-space-option]').forEach(button => {
        button.classList.toggle('is-active', button.dataset.collageSpaceOption === gutter);
    });
    document.querySelectorAll<HTMLElement>('[data-collage-corner-option]').forEach(button => {
        button.classList.toggle('is-active', button.dataset.collageCornerOption === radius);
    });
    document.querySelectorAll<HTMLElement>('[data-collage-color-target]').forEach(button => {
        button.classList.toggle('is-active', button.dataset.collageColorTarget === collageColorTarget);
    });
}

function bindCollageQuickControls(): void {
    document.querySelectorAll<HTMLElement>('[data-collage-popover]').forEach(button => {
    ...
    document.querySelectorAll<HTMLElement>('.collage-popover').forEach(menu => {
    ...
    document.querySelectorAll<HTMLElement>('[data-collage-fill]').forEach(button => {
        button.addEventListener('click', () => {
            const fill = button.dataset.collageFill || '#ffffff';
            if (collageIsGradientFill(fill)) collageSyncGradientStops(fill);
    ...
    const collageColorCodeInput = document.getElementById('collageColorCodeInput') as HTMLInputElement | null;
    const applyCollageCustomColor = () => {
        ...
        if (collageColorCodeInput) collageColorCodeInput.value = color;
        collageApplyPanelFill(color);
    };

    document.querySelector<HTMLElement>('[data-collage-custom-fill]')?.addEventListener('click', applyCollageCustomColor);
...
document.getElementById('collageBorderColor').addEventListener('change', function() {
    collageSetFill('border', (this as HTMLInputElement).value);
});

document.getElementById('collageBackground').addEventListener('change', function() {
    collageSetFill('background', (this as HTMLInputElement).value);
});
