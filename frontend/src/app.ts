...
document.addEventListener('DOMContentLoaded', function () {
      ...
      (async () => {
            ...
            async function tg_handleRunStopClick(event) {
                  ...
                  if (button.dataset.taskRunning === 'true') {
                        ...
                        } catch (error) { showToast(`Lá»—i khi dá»«ng: ${error.message}`, 'error'); }
                  } else {
            ...
            async function tg_handleRunGroupTask() {
                  ...
                  if (sessionsForTask.length === 0 && tg_currentTaskConfig.task === 'seedingGroup' && tg_currentTaskConfig.session_filenames?.length > 0) {
                        ...
                        showToast('KhÃ´ng cÃ³ session nÃ o Ä‘Æ°á»£c chá»n. Sá»­ dá»¥ng danh sÃ¡ch Ä‘Ã£ lÆ°u trong cáº¥u hÃ¬nh.', 'info');
                  }
                  if (sessionsForTask.length === 0) return showToast('Vui lÃ²ng chá»n hoáº·c lÆ°u session trong cáº¥u hÃ¬nh.', 'error');
                  ...
                  if (!payload.groupId) return showToast('Vui lÃ²ng chá»n nhÃ³m session.', 'error');

                  const taskDesc = telegramPane.querySelector('#tg-group-task-cards .card.card-selected .card-title')?.textContent.trim() || "tÃ¡c vá»¥";
                  tg_startTaskUI(sessionsForTask.length, `Báº¯t Ä‘áº§u "${taskDesc}"...`);
                  try {
                        ...
                        if (!response.ok) throw new Error((await response.json()).error || 'Lá»—i server.');
                        const { task_id } = await response.json();
                        tg_currentTaskId = task_id;
                        tg_setRunStopButtonState('running');
                        tg_pollTaskStatus(tg_currentTaskId);
                  } catch (error) { showToast(`Lá»—i: ${error.message}`, 'error'); tg_setRunStopButtonState('idle'); }
            }

            // START: Replacement for tg_handleCheckLive
            async function tg_handleCheckLive() {
                  if (tg_pollingInterval) return showToast('TÃ¡c vá»¥ khÃ¡c Ä‘ang cháº¡y.', 'error');

                  const groupId = document.getElementById('tg-group-session-select').value;
                  if (!groupId) return showToast('Vui lÃ²ng chá»n nhÃ³m.', 'error');

                  const selectedFilenames = Array.from(telegramPane.querySelectorAll('.tg-session-checkbox:checked:not(#tg-selectAllCheckbox):not(#tg-selectAllCheckbox-right)')).map(cb => cb.closest('tr').dataset.filename);
                  if (selectedFilenames.length === 0) return showToast('Vui lÃ²ng chá»n Ã­t nháº¥t má»™t session.', 'error');
                  ...
                  tg_startTaskUI(selectedFilenames.length, `Báº¯t Ä‘áº§u Check Live...`);
            ...
            function tg_startTaskUI(total, msg) {
                  ...
                  telegramPane.querySelectorAll('.tg-session-checkbox:checked').forEach(cb => {
                        ...
                        if (row) row.cells[6].innerHTML = `<span class="status-checking">Äang xá»­ lÃ½...</span>`;
                  });
            }
                  ...
                  tg_pollingInterval = setInterval(async () => {
                        ...
                        try {
                              ...
                              if (task.status === 'completed' || task.status === 'stopped') {
                                    ...
                                    showToast(task.status === 'completed' ? 'HoÃ n táº¥t tÃ¡c vá»¥!' : 'TÃ¡c vá»¥ Ä‘Ã£ dá»«ng.', 'success');
                                    document.getElementById('tg-status-progress-text').textContent = "Idle";
                                    tg_setRunStopButtonState('idle');
                                    tg_updateSessionCountDisplay();
                              }
                        } catch (error) { clearInterval(tg_pollingInterval); console.error('Lá»—i khi polling:', error); tg_setRunStopButtonState('idle'); }
                  }, 1500);
            }
            ...
            function tg_updateUiWithTaskProgress(task) {
                  ...
                  if (task.messages && task.messages.length > 0) {
                        ...
                        // --- NEW COUNTDOWN LOGIC ---
                        const countdownMatch = latestMessage.match(/Äang chá» Ä‘á»£t tiáº¿p...\s*(\d+s)/);
                        if (countdownMatch) {
                            const countdownText = `Chá»: ${countdownMatch[1]}`;
            ...
            async function tg_loadGroups() {
                  try {
                        ...
                        s.innerHTML = '<option selected disabled>-- Chá»n nhÃ³m --</option>';
                  ...
                  } catch (e) { showToast('KhÃ´ng thá»ƒ táº£i nhÃ³m Telegram.', 'error'); }
            }
            ...
            async function tg_handleGroupSelect(e) {
                  ...
                  try {
                        ...
                        leftBody.innerHTML = `<tr><td colspan="7" class="text-danger text-center">Lá»—i táº£i session</td></tr>`;
                  }
            }
            ...
            async function tg_populateSeedingGroupModal() {
                  ...
                  adminSelect.innerHTML = '<option value="">-- Äang táº£i... --</option>';
                  const adminGroup = tg_allGroups.find(g => g.name === 'Adminsession');
                  if (!adminGroup) { adminSelect.innerHTML = '<option value="">-- KhÃ´ng cÃ³ nhÃ³m Adminsession --</option>'; return; }
                  try {
                        const res = await fetch(`/telegram/api/groups/${adminGroup.id}/sessions`);
                        const adminSessions = await res.json();
                        adminSelect.innerHTML = '<option value="">-- KhÃ´ng sá»­ dá»¥ng Admin --</option>';
                  ...
                  } catch (error) { adminSelect.innerHTML = '<option value="">-- Lá»—i táº£i session --</option>'; }
            }
            ...
            async function tg_saveJoinGroupConfig() {
                  ...
                  try {
                        ...
                        showToast('ÄÃ£ lÆ°u cáº¥u hÃ¬nh Join Group!', 'success');
                  } catch (error) { showToast(`Lá»—i: ${error.message}`, 'error'); }
            }
            ...
            async function tg_saveSeedingGroupConfig() {
                  ...
                  try {
                        ...
                        showToast('ÄÃ£ lÆ°u cáº¥u hÃ¬nh Seeding Group!', 'success');
                  } catch (error) { showToast(`Lá»—i: ${error.message}`, 'error'); }
            }
            ...
            async function tg_handleSaveGroup(event) {
                  ...
                  if (!nameInput.value.trim() || !filesInput.files.length) return showToast('Vui lÃ²ng nháº­p tÃªn nhÃ³m vÃ  chá»n file.', 'error');
                  const formData = new FormData(form);
                  try {
                        const r = await fetch('/telegram/api/groups', { method: 'POST', body: formData });
                        const j = await r.json();
                        if (!r.ok) throw new Error(j.error || 'Lá»—i server.');
                        showToast(j.message || 'ThÃ nh cÃ´ng!', 'success');
            ...
            async function tg_resumeActiveTasks() {
                  try {
                        ...
                        if (taskIds.length > 0) {
                              ...
                              showToast(`KhÃ´i phá»¥c tráº¡ng thÃ¡i tÃ¡c vá»¥: ${taskData.task_name}`, 'info');
                  ...
                  } catch (error) { console.error("Lá»—i khi khÃ´i phá»¥c tÃ¡c vá»¥:", error); }
            }
            ...
            document.getElementById('tg-shuffleMessagesBtn').addEventListener('click', () => {
                  ...
                  showToast('ÄÃ£ xÃ¡o trá»™n tin nháº¯n!', 'success');
            });
            ...
            document.getElementById('tg-addSessionModal').addEventListener('show.bs.modal', async () => {
                  ...
                  try {
                        ...
                        listEl.innerHTML = groups.map(g => `<div class="d-flex justify-content-between align-items-center p-2 border-bottom"><span>${g.name}</span><button class="btn btn-sm btn-outline-danger tg-delete-group-btn" data-group-id="${g.id}"><i class="bi bi-trash-fill"></i></button></div>`).join('') || '<p class="text-muted text-center">ChÆ°a cÃ³ nhÃ³m nÃ o.</p>';
                  } catch (e) { listEl.innerHTML = `<div class="text-danger text-center">Lá»—i táº£i nhÃ³m.</div>`; }
            });
                  ...
                  if (deleteBtn) {
                        ...
                        window.confirmDeleteAction(async () => {
                              try {
                                    ...
                                    await tg_loadGroups();
                              } catch (err) { showToast('Lá»—i xÃ³a nhÃ³m', 'error'); }
                        }, 'Báº¡n cÃ³ cháº¯c muá»‘n xÃ³a nhÃ³m nÃ y?');
                        return;
                  }
            });
            ...
            if (proxyModalEl) {
                proxyModalEl.addEventListener('show.bs.modal', async () => {
                    try {
                        ...
                        showToast('Lá»—i táº£i cáº¥u hÃ¬nh proxy.', 'error');
                    }
                });
                ...
                saveProxyBtn.addEventListener('click', async () => {
                    try {
                        ...
                        const result = await response.json();
                        if (!response.ok) throw new Error(result.error || 'Lá»—i server');
                        showToast(result.message, 'success');
                        bootstrap.Modal.getInstance(proxyModalEl).hide();
                    } catch (error) {
                        showToast(`Lá»—i lÆ°u proxy: ${error.message}`, 'error');
                    }
                });
            }
      ...
      (() => {
            ...
            async function generateCollageBackground() {
                  ...
                  if (!selectedLayout) {
                        showToast('Vui lÃ²ng chá»n má»™t bá»‘ cá»¥c.', 'error');
                        return;
                  }
                  placeholder.classList.add('d-none');
                  showToast('Äang táº¡o áº£nh ná»n...', 'info');
            ...
            function addText() {
                  if (!backgroundImage) {
                        showToast('Vui lÃ²ng táº¡o áº£nh ghÃ©p trÆ°á»›c.', 'error');
                        return;
                  }
                  const newText = {
                        text: 'VÄƒn báº£n máº«u',
            ...
            function saveImage() {
                  if (!backgroundImage) {
                        showToast('KhÃ´ng cÃ³ áº£nh Ä‘á»ƒ lÆ°u.', 'error');
                        return;
                  }
            ...
            function updateThumbnails() {
                  ...
                  if (ie_uploadedFilesInfo.length === 0) {
                        thumbnailsContainer.innerHTML = '<div class="text-muted d-flex align-items-center justify-content-center h-100 p-2 text-center small">ChÆ°a cÃ³ áº£nh nÃ o</div>';
                  } else {
            ...
            function updateLayoutTemplates(count) {
                  ...
                  if (!layouts) {
                        layoutContainer.innerHTML = `<div class="text-warning text-center small">KhÃ´ng cÃ³ bá»‘ cá»¥c cho ${count} áº£nh.</div>`;
                        return;
                  }
                  let isFirst = true;
      ...
      (() => {
        ...
        async function fetchAndRenderNotes() {
            try {
                ...
                container.innerHTML = '';
                if (notes.length === 0) {
                    container.innerHTML = `<div class="col-12 text-center text-muted p-5"><h6>ChÆ°a cÃ³ ghi chÃº nÃ o.</h6><p>HÃ£y nháº¥n "ThÃªm Má»›i" Ä‘á»ƒ báº¯t Ä‘áº§u.</p></div>`;
                    return;
                }
                notes.forEach(note => container.appendChild(createNoteCard(note)));
            } catch (error) {
                container.innerHTML = `<div class="col-12 text-center text-danger p-5"><h6>Lá»—i táº£i ghi chÃº.</h6><p>${error.message}</p></div>`;
            }
        }
        ...
        function createNoteCard(note) {
            ...
            if (note.status === 'notified') {
                borderColor = '#28a745';
                statusHTML = `<small class="text-success fw-bold"><i class="bi bi-check-circle-fill"></i> ÄÃ£ bÃ¡o</small>`;
            } else if (note.due_time) {
                borderColor = '#ffc107';
                statusHTML = `<small class="text-warning fw-bold"><i class="bi bi-alarm-fill"></i> <span id="countdown-${note.id}" data-due-time="${note.due_time}">Äang tÃ­nh...</span></small>`;
            }

            const noteTitleText = note.title_html.trim() || "Ghi chÃº khÃ´ng cÃ³ tiÃªu Ä‘á»";
            ...
            if (modifiedAtElement && note.modified_at) {
                ...
                const formattedDate = `ÄÃ£ sá»­a ${date.getDate()}/${date.getMonth() + 1}/${date.getFullYear()}`;
        ...
        function updateCountdown(elementId, dueTimeString) {
            ...
            if (distance < 0) {
                elem.textContent = "ÄÃ£ Ä‘áº¿n giá»!";
                // KhÃ´ng cáº§n xÃ³a interval á»Ÿ Ä‘Ã¢y vÃ¬ server sáº½ sá»›m cáº­p nháº­t tráº¡ng thÃ¡i
                return;
            }
            ...
            if (hours > 0 || days > 0) result += `${hours} giá» `;
            if (minutes > 0 || hours > 0 || days > 0) result += `${minutes} phÃºt `;
            result += `${seconds} giÃ¢y`;

            elem.textContent = result;
        }

        // Cáº­p nháº­t táº¥t cáº£ cÃ¡c bá»™ Ä‘áº¿m ngÆ°á»£c má»—i giÃ¢y
        setInterval(() => {
        ...
        // --- END: Code má»›i thÃªm vÃ o ---

        // --- Modal and Form Logic ---
        window.prepareAddNoteModal = () => {
            form.reset();
            modalTitle.textContent = 'ThÃªm Ghi ChÃº Má»›i';
            editIdInput.value = '';
        ...
        window.prepareEditNoteModal = async (id) => {
            ...
            if (!note) {
                showAlert("KhÃ´ng thá»ƒ tÃ¬m tháº¥y ghi chÃº Ä‘á»ƒ sá»­a.");
                return;
            }

            form.reset();
            modalTitle.textContent = 'Sá»­a Ghi ChÃº';
        ...
        form.addEventListener('submit', async (e) => {
            ...
            if (!title_html && !content_html) {
                showAlert('TiÃªu Ä‘á» hoáº·c Ná»™i dung khÃ´ng Ä‘Æ°á»£c Ä‘á»ƒ trá»‘ng.');
                return;
            }
            const payload = {
            ...
            if (!response.ok) {
                showAlert('Lá»—i: ' + (await response.json()).error);
                return;
            }
            notesModal.hide();
            fetchAndRenderNotes();
        ...
        setInterval(async () => {
            try {
                ...
                if (notification) {
                    ...
                    notificationContent.innerHTML = notification.notes || 'KhÃ´ng cÃ³ ná»™i dung chi tiáº¿t.';
        ...
        // Thay tháº¿ listener cÅ© báº±ng listener nÃ y
            titleInput.addEventListener('contextmenu', e => {
                  e.preventDefault();
                  e.stopPropagation();
                  const selection = window.getSelection();
                  // Chá»‰ hiá»ƒn thá»‹ menu náº¿u cÃ³ Ä‘oáº¡n text Ä‘Æ°á»£c bÃ´i Ä‘en
                  if (selection.toString().length > 0) {
                        // ÄÃ¢y lÃ  logic Ä‘Ãºng, giá»‘ng há»‡t vá»›i cá»§a Ã´ Ná»™i dung
        ...
        function updateNotesHeader(activeTabId) {
            if (activeTabId === 'notes-sub-tab-mxh') {
                // --- State for MXH Tab ---
                notesMainTitle.innerHTML = `<i class="bi bi-people-fill me-2"></i>Máº¡ng XÃ£ Há»™i`;
                addNewBtn.innerHTML = `<i class="bi bi-plus-lg me-1"></i> ThÃªm TÃ i Khoáº£n MXH`;
                ...
                notesMainTitle.innerHTML = `<i class="bi bi-journal-richtext me-2"></i>Ghi ChÃº`;
                addNewBtn.innerHTML = `<i class="bi bi-plus-lg me-1"></i> ThÃªm Má»›i`;
...
(function() {
  ...
  document.addEventListener('submit', function(e) {
    ...
    if (form.classList.contains('needs-confirm-delete')) {
      ...
      const msg = form.getAttribute('data-confirm-message') || 'Báº¡n cÃ³ cháº¯c muá»‘n xÃ³a?';
  ...
  window.confirmDeleteAction = function(callback, message) {
    ...
    document.getElementById('globalDeleteConfirmText').textContent = message || 'Báº¡n cÃ³ cháº¯c muá»‘n xÃ³a?';
    new bootstrap.Modal(document.getElementById('globalDeleteConfirmModal')).show();
  };
...
if (autoStartSwitch) {
    autoStartSwitch.addEventListener('change', async () => {
        try {
            ...
            if (response.ok) {
                showToast("ÄÃ£ lÆ°u cÃ i Ä‘áº·t tá»± khá»Ÿi Ä‘á»™ng.", "success");
            } else {
                throw new Error("Lá»—i server.");
            }
        } catch (error) {
            console.error("Lá»—i khi lÆ°u cÃ i Ä‘áº·t:", error);
            showToast("KhÃ´ng thá»ƒ lÆ°u cÃ i Ä‘áº·t. Vui lÃ²ng thá»­ láº¡i.", "error");
