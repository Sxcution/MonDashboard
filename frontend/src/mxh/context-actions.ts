...
(function () {
    ...
    async function handleContextAction(ctx, event) {
        ...
        if (!menuItem) return;
        if (menuItem.closest('#mxh-background-context-menu')) return;
    ...
    function bind(ctx) {
        ...
        if (doc.documentElement?.dataset.mxhContextActionsBound) return;
        if (doc.documentElement) doc.documentElement.dataset.mxhContextActionsBound = '1';
