...
(function () {
    ...
    function bindSubmenuPositioning(doc = document) {
        if (doc.documentElement?.dataset.mxhContextSubmenuBound) return;
        if (doc.documentElement) doc.documentElement.dataset.mxhContextSubmenuBound = '1';
        ...
        doc.addEventListener('mouseover', function (e) {
            ...
            const submenu = item.querySelector(':scope > .mxh-submenu') as HTMLElement | null;
            if (!submenu) return;
    ...
    function updateBackgroundViewChecks(ctx) {
        ...
        if (viewChecks.default) viewChecks.default.textContent = ctx.activeViewFilter === 'default' ? '✓ ' : '';
        if (viewChecks.card2) viewChecks.card2.textContent = ctx.activeViewFilter === 'card2' ? '✓ ' : '';
        if (viewChecks.card3) viewChecks.card3.textContent = ctx.activeViewFilter === 'card3' ? '✓ ' : '';
    }
    ...
    function showBackgroundContextMenu(ctx, event) {
        ...
        const existingMenu = doc.getElementById('card-context-menu');
        if (existingMenu) hideCardContextMenu(ctx);
