...
(function () {
    function createRuntime(ctx) {
        ...
        function bindGlobalEvents() {
            ...
            doc.addEventListener('focusin', (event) => {
                if (isEditableElement(event.target)) setActive(true);
            });
            ...
            doc.addEventListener('selectionchange', () => {
                if (isEditableElement(doc.activeElement)) captureSelection();
            });

            doc.addEventListener('input', (event) => {
                if (isEditableElement(event.target)) captureSelection();
            });
            ...
            win.addEventListener('focus', () => {
                setTimeout(() => {
                    if (!doc.hidden && active) restoreSelection();
                    if (!doc.hidden && !isEditableElement(doc.activeElement)) {
                        active = false;
                    }
                }, 250);
    ...
    function formatDisplay(field, value) {
        ...
        if (field === 'phone') return hasValue ? `📞 ${normalized}` : '📞 ...';
        if (field === 'email') return hasValue ? `✉ ${normalized}` : '✉ ...';
        if (field === 'wechat_nickname') return hasValue ? `@ ${normalized}` : '@ ...';
