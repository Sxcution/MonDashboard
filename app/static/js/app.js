...
document.addEventListener('DOMContentLoaded', function () {
    ...
    document.querySelectorAll('button[data-bs-toggle="tab"]').forEach(tabEl => {
        tabEl.addEventListener('shown.bs.tab', function (event) {
            if (event.target.id.includes('-v-pills-'))
                return;
    ...
    (async () => {
        ...
        async function tg_handleRunStopClick(event) {
            ...
            if (button.dataset.taskRunning === 'true') {
                ...
                try {
                    ...
                    showToast(`Lá»—i khi dá»«ng: ${error.message}`, 'error');
                }
            }
            else {
        ...
        async function tg_handleRunGroupTask() {
            ...
            if (sessionsForTask.length === 0)
                return showToast('Vui lÃ²ng chá»n hoáº·c lÆ°u session trong cáº¥u hÃ¬nh.', 'error');
            const payload = {
            ...
            if (!payload.groupId)
                return showToast('Vui lÃ²ng chá»n nhÃ³m session.', 'error');
            ...
            try {
                ...
                if (!response.ok)
                    throw new Error((await response.json()).error || 'Lá»—i server.');
        ...
        async function tg_handleCheckLive() {
            if (tg_pollingInterval)
                return showToast('TÃ¡c vá»¥ khÃ¡c Ä‘ang cháº¡y.', 'error');
            const groupId = document.getElementById('tg-group-session-select').value;
            if (!groupId)
                return showToast('Vui lÃ²ng chá»n nhÃ³m.', 'error');
            const selectedFilenames = Array.from(telegramPane.querySelectorAll('.tg-session-checkbox:checked:not(#tg-selectAllCheckbox):not(#tg-selectAllCheckbox-right)')).map(cb => cb.closest('tr').dataset.filename);
            if (selectedFilenames.length === 0)
                return showToast('Vui lÃ²ng chá»n Ã­t nháº¥t má»™t session.', 'error');
        ...
        function tg_startTaskUI(total, msg) {
            ...
            telegramPane.querySelectorAll('.tg-session-checkbox:checked').forEach(cb => {
                ...
                if (row)
                    row.cells[6].innerHTML = `<span class="status-checking">Äang xá»­ lÃ½...</span>`;
            });
        }
        ...
        function tg_pollTaskStatus(taskId) {
            ...
            tg_pollingInterval = setInterval(async () => {
                ...
                try {
                    ...
                    clearInterval(tg_pollingInterval);
                    console.error('Lá»—i khi polling:', error);
                    tg_setRunStopButtonState('idle');
                }
            }, 1500);
        }
        ...
        async function tg_loadGroups() {
            try {
                ...
                showToast('KhÃ´ng thá»ƒ táº£i nhÃ³m Telegram.', 'error');
            }
        }
        ...
        async function tg_populateSeedingGroupModal() {
            ...
            if (!adminGroup) {
                adminSelect.innerHTML = '<option value="">-- KhÃ´ng cÃ³ nhÃ³m Adminsession --</option>';
                return;
            }
            try {
                ...
                adminSelect.innerHTML = '<option value="">-- Lá»—i táº£i session --</option>';
            }
        }
        ...
        async function tg_handleSaveGroup(event) {
            ...
            if (!nameInput.value.trim() || !filesInput.files.length)
                return showToast('Vui lÃ²ng nháº­p tÃªn nhÃ³m vÃ  chá»n file.', 'error');
            const formData = new FormData(form);
            try {
                const r = await fetch('/telegram/api/groups', { method: 'POST', body: formData });
                const j = await r.json();
                if (!r.ok)
                    throw new Error(j.error || 'Lá»—i server.');
        ...
        async function tg_resumeActiveTasks() {
            try {
                ...
                console.error("Lá»—i khi khÃ´i phá»¥c tÃ¡c vá»¥:", error);
            }
        }
        ...
        document.getElementById('tg-addSessionModal').addEventListener('show.bs.modal', async () => {
            ...
            try {
                ...
                listEl.innerHTML = `<div class="text-danger text-center">Lá»—i táº£i nhÃ³m.</div>`;
            }
        });
            ...
            if (deleteBtn) {
                ...
                window.confirmDeleteAction(async () => {
                    try {
                        ...
                        showToast('Lá»—i xÃ³a nhÃ³m', 'error');
                    }
        ...
        if (proxyModalEl) {
            ...
            saveProxyBtn.addEventListener('click', async () => {
                try {
                    ...
                    const result = await response.json();
                    if (!response.ok)
                        throw new Error(result.error || 'Lá»—i server');
                    showToast(result.message, 'success');
        ...
        function drawTextObject(obj) {
            ...
            lines.forEach((line, i) => {
                ...
                if (obj.textDecoration === 'underline') {
                    ...
                    if (ctx.textAlign === 'center') {
                        underlineX = obj.x + (obj.width - textWidth) / 2;
                    }
                    if (ctx.textAlign === 'right') {
                        underlineX = obj.x + (obj.width - textWidth);
                    }
        ...
        function updateSelectedTextProperty(key, value) {
            if (selectedTextIndex > -1) {
                ...
                if (key === 'fontWeight' || key === 'fontStyle' || key === 'textDecoration') {
                    ...
                    if (key === 'textDecoration' && obj[key] === 'normal')
                        obj[key] = 'none';
                }
                else {
                    obj[key] = value;
    ...
    (function () {
        ...
        function showMenu(menu, x, y) {
            ...
            if (x + menuRect.width > winW)
                x = winW - menuRect.width - 8;
            if (y + menuRect.height > winH)
                y = winH - menuRect.height - 8;
