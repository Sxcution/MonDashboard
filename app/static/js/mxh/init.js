...
(function () {
    ...
    function checkNoticeExpirations(ctx) {
        if (typeof ctx.window.showPlatformNotification !== 'function')
            return;
    ...
    function bindModalLifecycle(ctx) {
        ctx.document.querySelectorAll('.modal').forEach(modal => {
            if (modal.dataset.mxhLifecycleBound)
                return;
    ...
    function bindVisibility(ctx) {
        const doc = ctx.document;
        if (doc.documentElement?.dataset.mxhVisibilityBound)
            return;
        if (doc.documentElement)
            doc.documentElement.dataset.mxhVisibilityBound = '1';
    ...
    function bindFilterDropdown(ctx) {
        ...
        if (!filterMenu || filterMenu.dataset.mxhFilterBound)
            return;
    ...
    function bindSearch(ctx) {
        ...
        if (!searchInput || !clearButton || !searchGroup || searchGroup.dataset.mxhSearchBound)
            return;
