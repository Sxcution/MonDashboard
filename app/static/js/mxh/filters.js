...
(function () {
    function getNearbyPeopleHoursUntilActive(ctx, account, now) {
        ...
        if ((account.status || '').toLowerCase() === 'disabled')
            return Number.POSITIVE_INFINITY;
        if (!ctx.isEligibleNearbyPeople(account))
            return Number.POSITIVE_INFINITY;
    ...
    function getNearestNearbyPeopleHours(ctx, accounts, now) {
        ...
        accounts.forEach(account => {
            ...
            if (hoursUntilActive < nearestHours)
                nearestHours = hoursUntilActive;
        });
        return nearestHours;
    }
    function applyQuickFilter(ctx, filterKey) {
        ...
        const dropdownItems = ctx.document.querySelectorAll('.dropdown-item');
    ...
    function updateCardVisibility(ctx, searchQuery) {
        ...
        const filterDims = (ctx.activeFilter === 'wechat_scan' ||
            ...
            if (shouldSortByCreationDate) {
                ...
                if (ctx.activeFilter === 'creation_date') {
                    ...
                    accounts.forEach(account => {
                        ...
                        if (date && !isNaN(date.getTime()) && date.getTime() < minTs)
                            minTs = date.getTime();
                    });
                    ...
                    accounts.forEach(account => {
                        const date = ctx.getAccountCreatedDateForStats(account);
                        if (date && !isNaN(date.getTime()) && date.getTime() > maxTs)
                            maxTs = date.getTime();
                    });
