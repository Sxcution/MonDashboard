...
(function () {
    function init(ctx) {
        ...
        if (doc.documentElement?.dataset.mxhNoticePreviewBound)
            return;
        if (doc.documentElement)
            doc.documentElement.dataset.mxhNoticePreviewBound = '1';
        let $preview = null;
        let $badge = null;
        ...
        function onDocumentClick(event) {
            if (!$preview || !$badge)
                return;
            if (event.target.closest('.notice-preview') || event.target.closest('.notice-badge'))
                return;
            closePreview(true);
        }
        ...
        function buildPreviewDOM(data, badgeType, mode) {
            ...
            box.addEventListener('mouseover', () => { if (hoverLeaveTimer) {
                clearTimeout(hoverLeaveTimer);
                hoverLeaveTimer = null;
            } });
        ...
        win.addEventListener('scroll', () => { if ($preview && $badge)
            placePreview($preview, $badge); }, true);
        win.addEventListener('resize', () => { if ($preview && $badge)
            placePreview($preview, $badge); }, true);
    }
