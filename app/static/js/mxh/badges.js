    ...
    function getCardBadge(ctx, account, allAccounts = []) {
        ...
        cardAccounts.forEach((acc, idx) => {
            ...
            if (acc.platform === 'wechat' && acc.wechat_created_year) {
                const createdDate = new Date(acc.wechat_created_year, (acc.wechat_created_month || 1) - 1, acc.wechat_created_day || 1);
        ...
        if (badgeInfo.noticeExpired > 0) {
            const accountsList = badgeInfo.noticeExpiredAccounts.map(a => `Tài khoản thứ ${a.index}`).join(', ');
            let noticeCache = '';
            if (account.notice) {
                try {
        ...
        if (badgeInfo.needHongKong > 0 && badgeInfo.noticeExpired === 0) {
            const accountsList = badgeInfo.needHongKongAccounts.map(a => `Tài khoản thứ ${a.index}`).join(', ');
    ...
    function calculateGroupBadge(ctx, groupId) {
        ...
        ctx.mxhAccounts.forEach(account => {
            ...
            if (account.platform === 'wechat' && account.secondary_wechat_created_year) {
                const secCreatedDate = new Date(account.secondary_wechat_created_year, (account.secondary_wechat_created_month || 1) - 1, account.secondary_wechat_created_day || 1);
