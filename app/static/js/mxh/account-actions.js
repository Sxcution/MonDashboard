...
(function () {
    ...
    document.getElementById('move-account-confirm-btn').addEventListener('click', async () => {
        ...
        const targetAccount = ctx.mxhAccounts.find(acc => acc.card_name === targetCardName && acc.group_id === account.group_id);
        if (!targetAccount) {
    ...
    async function deleteCard(cardId) {
        if (!(await ctx.confirm('Xóa card này và tất cả tài khoản trên card?')))
            return;
        try {
    ...
    async function deleteSubAccount(accountId) {
        if (!(await ctx.confirm('Xóa tài khoản phụ này?')))
            return;
        try {
