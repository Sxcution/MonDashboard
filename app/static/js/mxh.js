...
(function () {
    ...
    if (mxhAccountsContainer) {
        ...
        mxhAccountsContainer.addEventListener('contextmenu', (event) => {
            ...
            if (!card || !mxhAccountsContainer.contains(card))
                return;
            window.handleCardContextMenu(event, Number(card.dataset.cardId), Number(card.dataset.accountId), card.dataset.platform);
        });
        mxhAccountsContainer.addEventListener('keydown', (event) => {
            const field = event.target?.closest('.editable-field[contenteditable="true"]');
            if (!field || !mxhAccountsContainer.contains(field))
                return;
            if (event.key === 'Enter') {
    ...
    async function loadMXHData(forceRender = false) {
        if (mxhInlineEditRuntime.isEditing())
            return; // Keep caret/selection stable while editing inline fields.
        if (window.interactionPaused || document.hidden)
            return; // (1) chặn khi đang tương tác/ẩn tab
        ...
        try {
            ...
            if (accountsResponse.ok) {
                ...
                if (touchedAny) {
                    ...
                    lastUpdateTime = Math.max(lastUpdateTime || 0, ...(delta.map(acc => new Date(acc.updated_at || acc.updatedAt || Date.now()).getTime()))) || Date.now();
                }
            }
            ...
            if (err.name !== 'AbortError')
                console.error('loadMXHData error:', err);
        }
    }
    // ===== AUTO-REFRESH SYSTEM =====
    ...
    async function handleNearbyAction(accountId, actionType) {
        try {
            ...
            if (data.success) {
                if (typeof showToast === 'function')
                    showToast('Cập nhật Nearby People thành công', 'success');
                loadMXHData(true);
            }
            else {
                if (typeof showToast === 'function')
                    showToast('Lỗi: ' + (data.error || 'Server error'), 'error');
            }
        }
        catch (e) {
