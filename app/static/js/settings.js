...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    document.querySelectorAll('[data-settings-action]').forEach((element) => {
        element.addEventListener('click', (event) => {
            ...
            if (action === 'reload')
                loadSettings(true);
            if (action === 'enable-notification')
                requestNotificationPermission();
            if (action === 'apply-shutdown')
                applyShutdownTimer();
            if (action === 'apply-notification')
                applyNotificationTimer();
        });
        element.addEventListener('change', (event) => {
            const action = event.currentTarget.dataset.settingsAction;
            if (action === 'toggle-auto-start')
                toggleAutoStart();
            if (action === 'toggle-auto-open-dashboard')
                toggleAutoOpenDashboard();
            if (action === 'toggle-shutdown')
                toggleShutdownTimer();
