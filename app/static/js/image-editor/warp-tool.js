...
let warpGridSize = 16;
let warpSettingsTimeout = null;
let warpBaseImageData = null;
let warpCurrentImageData = null;
let warpPreviewCanvas = null;
let warpPreviewCtx = null;
let warpCursor = null;
let warpLastPoint = null;
let warpDirty = false;
let warpHistory = [];
// Intercept global deactivateAllTools to include warp tool deactivation
(function () {
    const originalDeactivateAllTools = window.deactivateAllTools || globalThis.deactivateAllTools;
    ...
    window.deactivateAllTools = newDeactivateAllTools;
    globalThis.deactivateAllTools = newDeactivateAllTools;
})();
// Settings Menu Visibility Toggle
function showWarpSettings() {
...
function toggleWarpTool() {
    ...
    if (!warpToolActive) {
        ...
        if (singleImageCanvas && singleImageCtx) {
            ...
            warpCurrentImageData = new ImageData(new Uint8ClampedArray(warpBaseImageData.data), warpBaseImageData.width, warpBaseImageData.height);
            warpHistory = [
                new ImageData(new Uint8ClampedArray(warpBaseImageData.data), warpBaseImageData.width, warpBaseImageData.height)
            ];
        }
        warpDirty = false;
        setupWarpListeners();
...
function resetWarpPreview() {
    if (!warpToolActive || !warpBaseImageData || !singleImageCtx)
        return;
    singleImageCtx.putImageData(warpBaseImageData, 0, 0);
    warpCurrentImageData = new ImageData(new Uint8ClampedArray(warpBaseImageData.data), warpBaseImageData.width, warpBaseImageData.height);
    warpHistory = [
...
function continueWarpDrag(event) {
    if (!warpToolActive || !isWarpDragging || !warpLastPoint)
        return;
    if (event.cancelable)
        event.preventDefault();
...
function endWarpDrag() {
    ...
    if (warpCurrentImageData) {
        warpHistory.push(new ImageData(new Uint8ClampedArray(warpCurrentImageData.data), warpCurrentImageData.width, warpCurrentImageData.height));
...
function updateWarpCursor() {
    if (!warpCursor || !singleImageCanvas)
        return;
    const rect = singleImageCanvas.getBoundingClientRect();
...
function applyMeshWarpStroke(from, to) {
    if (!warpCurrentImageData || !singleImageCanvas || !singleImageCtx)
        return;
    ...
    const src = new ImageData(new Uint8ClampedArray(warpCurrentImageData.data), width, height);
    const out = new ImageData(new Uint8ClampedArray(warpCurrentImageData.data), width, height);
...
function renderWarpedPatch(src, out, from, dragX, dragY, radius, sigma, strength, minX, maxX, minY, maxY) {
...
function commitWarpPreviewToCanvas() {
    if (!singleImageCanvas)
        return;
    ...
    if ((event.ctrlKey || event.metaKey) && event.key === 'z' && !event.shiftKey) {
        if (warpToolActive) {
            ...
            if (warpHistory.length > 1) {
                ...
                warpCurrentImageData = new ImageData(new Uint8ClampedArray(prevState.data), prevState.width, prevState.height);
                if (singleImageCtx) {
