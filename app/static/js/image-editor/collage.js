...
function collageBuildCheckpointGradient() {
    const handles = Array.from(document.querySelectorAll('#collageQuickPanel .gradient-checkpoint-handle'));
    if (handles.length === 0) {
...
function collageSyncGradientStops(fill) {
    ...
    if (handles.length > 0) {
        handles.forEach((handle, index) => {
            ...
            const pin = handle.querySelector('.teardrop-pin');
            if (pin) {
...
window.initializeGradientEditor = function (editorContainer, onUpdate) {
    const checkpointsContainer = editorContainer.querySelector('.image-gradient-checkpoints');
    ...
    const inputs = Array.from(checkpointsContainer.querySelectorAll('input[type="color"]'));
    const defaultPositions = [0, 50, 100];
