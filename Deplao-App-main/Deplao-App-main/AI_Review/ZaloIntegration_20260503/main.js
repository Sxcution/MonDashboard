...
function createWindow() {
  ...
  // ============================================================
  //  GLOBAL POLLING: Auto-click and fallback title check
  // ============================================================
  const BADGE_POLL_SCRIPT = [
    '(function() {',
    '  // === AUTO-CLICK Kich hoat BUTTON FOR ZALO ===',
    '  if (window.location.hostname.includes("zalo.me")) {',
    '    var allEls = document.querySelectorAll("button, div, span, a");',
    '    for (var i = 0; i < allEls.length; i++) {',
    '      var el = allEls[i];',
    '      if (el.textContent && el.textContent.trim() === "K\u00edch ho\u1ea1t" && el.offsetParent !== null) {',
    '        el.click();',
    '        break;',
    '      }',
    '    }',
  ...
  setInterval(async () => {
    for (const [profileId, view] of Object.entries(browserViews)) {
      ...
      try {
        const count = await view.webContents.executeJavaScript(BADGE_POLL_SCRIPT, true);
        if (mainWindow && !mainWindow.isDestroyed()) {
          mainWindow.webContents.send('update-profile-badge', { id: profileId, count: count || 0 });
        }
      } catch (e) { /* page still loading or view destroyed */ }
    }
  }, 3000);
